...
import { StyledDropdown, StyledDropdownIcon, StyledDropdownOptionMenu } from './styles'
...
export const Dropdown = forwardRef(({
  ...
  ...rest
}: DropdownProps, fRef: ForwardedRef<HTMLDivElement>) => {
    ...
    return values.length === 0
      ? placeholder : (
        values.length > 3
          ...
          : (children as any[]).filter(c => valueSet.has(c.props.value)).map(c => (
            <span key={c.props.value} className='mr-md'>{c.props.children}</span>
          ))
      )
    ...
    <DropdownContext.Provider value={contextValue}>
      <StyledInputWrapper>
        ...
        <StyledDropdown {...{ id, ref, onClick, isOpen, ...rest }}>
          {selectedText}<StyledDropdownIcon />
