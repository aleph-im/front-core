  ...
  <>
    ...
    <Row {...args}>
      { Array.from({ length: 4 }, (_, i) => i + 1).map(i => (
        <Col span={3}><div style={colStyle}>{i}</div></Col>
      )) }
    </Row>
    
    <h1>Regular span blocks (size=1)</h1>
    <Row {...args}>
      { Array.from({ length: args.count || defaultCount }, (_, i) => i + 1).map(i => (
        <Col span={1}><div style={colStyle}>{i}</div></Col>
      )) }
    </Row>
    
    <h1>Custom offset blocks</h1>
    <Row {...args}>
      <Col span={2} offset={3}><div style={colStyle}>1</div></Col>
      <Col span={4} offset={7}><div style={colStyle}>2</div></Col>
    </Row>

    <h1>Irregular span blocks</h1>
    <Row {...args}>
      <Col span={4}><div style={colStyle}>1</div></Col>
      <Col span={8}><div style={colStyle}>2</div></Col>
    </Row>
  </>
);
