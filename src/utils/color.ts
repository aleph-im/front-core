...
export function colorNameToHex(colorName: string): string | undefined {
  return defaultBrowserColors[colorName]
}

export function colorHexToRGBA(hex: string): [number, number, number, number] {
  hex = hex.replace('#', '')

  const r = parseInt(hex.slice(0, 2), 16)
  const g = parseInt(hex.slice(2, 4), 16)
  const b = parseInt(hex.slice(4, 6), 16)
  const alpha = hex.length > 6 ? parseInt(hex.slice(4, 6), 16) : 255

  return [r, g, b, alpha]
}

export function colorHexToRGBALottie(
  hex: string,
